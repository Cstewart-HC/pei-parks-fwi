...
class TestAC_PIPE_4_StanhopeValidation:
    ...
    def test_stanhope_validation_report_exists(self):
        ...
        pytest.skip("stanhope_validation.csv not yet generated by pipeline")
...
class TestAC_PIPE_5_QAQCReporting:
    ...
    def test_qa_qc_report_exists(self):
        """AC-QC-1: qa_qc_report_hourly.csv exists."""
        qa_qc_path = DATA_PROCESSED / "qa_qc_report_hourly.csv"
        assert qa_qc_path.exists(), "Missing qa_qc_report_hourly.csv"
...
class TestAC_PIPE_6_Determinism:
    ...
    def test_column_order_is_deterministic(self):
        ...
        if len(columns) > 3:
            filtered = [c for c in columns[2:] if c != "can_compute"]
            sorted_filtered = [c for c in sorted_columns[2:] if c != "can_compute"]
            assert filtered == sorted_filtered, (
